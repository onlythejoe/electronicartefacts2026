(function () {
  ...
  const homeHeroFrame = (item, className, options = {}) => {
    ...
    const description = options.description || item.abstract || item.description || "";
    const status = options.status || item.statusLabel || item.status || "Active";
    const visual = options.visual || (media
    ...
    return `
      <article class="home-intent-stage__frame home-project-hologram ${esc(className)}" data-entry-id="${esc(item.id || "")}" data-depth="${esc(options.depth || 1)}" data-home-project-card tabindex="0" aria-label="${esc(`${options.previewLabel || "Project preview"}: ${item.title}`)}">
        ...
        <span class="home-project-hologram__controls" role="group" aria-label="${esc(options.controlsLabel || "Resize project preview")}">
          ...
          <button type="button" data-home-card-size="larger" aria-label="${esc(options.largerLabel || "Make preview larger")}">+</button>
        </span>
        <button class="home-project-hologram__resize-handle" type="button" data-home-card-resize aria-label="${esc(options.resizeLabel || "Resize project preview")}"><span></span></button>
      </article>
    `;
  };
  const renderHomeHero = () => {
    const ranked = homeProjects();
  ...
  const initPageInteractions = () => {
    ...
    document.querySelectorAll("[data-selected-works-toggle]").forEach((button) => {
      ...
      button.addEventListener("click", () => {
        ...
        if (!expanded) refreshCardSurfaces(target);
      });
    });
  };
